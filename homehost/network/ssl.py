...
def check_ssl_cert(hostname: str, port: int = 443) -> dict[str, Any]:
    ...
    try:
        with (
            socket.create_connection((hostname, port), timeout=_SSL_TIMEOUT) as raw_sock,
            context.wrap_socket(raw_sock, server_hostname=hostname) as ssl_sock,
        ):
...
def days_until_expiry(hostname: str, port: int = 443) -> int:
    ...
    if not cert_info["valid"] and not cert_info["expires"]:
        log.debug("days_until_expiry(%r): cert check failed: %s", hostname, cert_info["error"])
        return -1
